...
# ~ your email address (hereafter: ccohane@gmail.com)                               #
# ~ your server's internet protocol address (hereafter: 104.248.112.96)         #
# ~ your server's name (hereafter: server-1)                                 #
#                                                                              #
# Be advised:                                                                  #
# ~ ccohane must be a string that is not "root"                          #
# ~ callahan should be a string that is longer than eight characters      #
# ~ 45934 must be an integer that is between 1024 and 65535       #
#                                                                              #
################################################################################
...
sh -c 'echo "ccohane:callahan" > .credentials'

ssh root@104.248.112.96
...
adduser --disabled-password --gecos "" ccohane

usermod -aG sudo ccohane

cp .nanorc /home/ccohane/

mkdir /etc/ssh/ccohane
...
scp ~/.ssh/id_rsa.pub root@104.248.112.96:/etc/ssh/ccohane/authorized_keys

scp .credentials root@104.248.112.96:/home/ccohane/
...
apt-get -y install fail2ban firewalld nginx ntp 

chown -R ccohane:ccohane /etc/ssh/ccohane

chmod 755 /etc/ssh/ccohane

chmod 644 /etc/ssh/ccohane/authorized_keys

sed -i -e '/^#AuthorizedKeysFile/s/^.*$/AuthorizedKeysFile \/etc\/ssh\/ccohane\/authorized_keys/' /etc/ssh/sshd_config
...
sh -c 'echo "AllowUsers ccohane" >> /etc/ssh/sshd_config'
...
sed -i -e '/^Port/s/^.*$/Port 45934/' /etc/ssh/sshd_config

firewall-cmd --add-port 45934/tcp --permanent
...
sh -c 'echo "destemail = ccohane@gmail.com" >> /etc/fail2ban/jail.local'

sh -c 'echo "sendername = security@server-1" >> /etc/fail2ban/jail.local'
...
cat /home/ccohane/.credentials | chpasswd

rm /home/ccohane/.credentials
...
# CREATE USER ccohane WITH PASSWORD 'callahan';

# CREATE DATABASE master OWNER ccohane;
...
# git clone git://github.com/greg/project_name.git
